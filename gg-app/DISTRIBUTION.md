...
This is upstream distribution documentation. The fork imports the source but
restricts publication jobs to `KenKaiii/gg-framework`. Local integration does not
authorize signing, downloads of system runtimes, publication, or deployment.
...
```bash
pnpm --filter @kleio/coder build      # produce dist/app-sidecar.js
