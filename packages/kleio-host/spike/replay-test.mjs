...
const H = {
  host: process.env.KLEIO_HOST ?? "127.0.0.1",
  port: Number(process.env.KLEIO_PORT ?? 18443),
};
  ...
  new Promise((res, rej) => {
    ...
    const r = http.request(
      ...
      (u) => {
        let s = "";
        u.on("data", (d) => (s += d));
        u.on("end", () => res({ status: u.statusCode, body: s }));
      },
    );
    r.on("error", rej);
    r.end(data);
...
const record = (id, f) => {
  ...
  if (f.type === "text_delta")
    deltas.set(id, typeof f.data === "string" ? f.data : (f.data?.text ?? f.data?.delta ?? ""));
  if (
    f.type === "agent_end" ||
    (f.type === "state" && f.data?.runState === "idle" && seen.size > 5)
  )
    done = true;
};
  ...
  req1 = http.request({ ...H, path: `/events?session=${SID}`, headers }, (res) => {
    ...
    res.on("data", (c) => {
      ...
      while ((i = buf.indexOf("\n\n")) !== -1) {
        ...
        const data = raw.match(/^data: (.*)$/m)?.[1];
        if (id && data) {
          phase1Last = Number(id);
          record(Number(id), JSON.parse(data));
        }
      }
    });
    res.on("close", resolve);
...
console.log(
  "prompt →",
  await post("/prompt", {
    text: "Count from 1 to 400, one number per line, no commentary, no grouping.",
  }),
);
    ...
    res.on("data", (c) => {
      ...
      while ((i = buf.indexOf("\n\n")) !== -1) {
        ...
        if (id && data) {
          ...
          record(Number(id), f);
          if (f.type === "kleio_replay_gap") console.log("GAP", f.data);
          if (f.type === "run_end" || f.type === "agent_end" || f.type === "turn_end") {
            req.destroy();
          }
        }
      }
...
const p2 = ids.filter((id) => id > phase1Last);
const gaps = p2.filter((id, i) => i > 0 && id !== p2[i - 1] + 1);
console.log(
  `phase-2 ids ${p2[0]}..${p2.at(-1)} contiguous=${gaps.length === 0} first-after-drop=${p2[0] === phase1Last + 1}`,
);
...
const hist = await new Promise((res) => {
  http.get({ ...H, path: "/history", headers: auth }, (u) => {
    let s = "";
    u.on("data", (d) => (s += d));
    u.on("end", () => res(JSON.parse(s).history));
  });
});
const last = hist.at(-1);
const txt = typeof last?.text === "string" ? last.text : JSON.stringify(last).slice(0, 200);
const nums = (txt.match(/\b\d+\b/g) || []).map(Number);
const streamed = [...deltas.entries()]
  .sort((a, b) => a[0] - b[0])
  .map(([, t]) => t)
  .join("");
console.log(
  `streamed text == history text: ${streamed === txt} (streamed ${streamed.length} chars, history ${txt.length})`,
);
console.log(
  `history: ${hist.length} entries; last role=${last?.role} numbers found=${nums.length} first=${nums[0]} last=${nums.at(-1)} monotonic=${nums.every((n, i) => i === 0 || n === nums[i - 1] + 1)}`,
);
