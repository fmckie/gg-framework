...
export interface AgentLoopOptions {
  ...
  model: string;
  tools: AgentTool[];
  /** Snapshot current project-bound resources at run time, including before React commits. */
  getProjectRuntime?: () => Pick<
    AgentLoopOptions,
    "tools" | "lspManager" | "reviewCoverageTracker"
  >;
  webSearch?: boolean;
  maxTokens: number;
...
export interface UseAgentLoopReturn {
  ...
  abort: () => void;
  /** Suspend new runs and await active finalizers; the returned function resumes input. */
  suspendForProjectSwitch?: () => Promise<() => void>;
  reset: () => void;
...
export function useAgentLoop(
  ...
  const runCompletionRef = useRef(new RunCompletion());
  ...
  const run = useCallback(
    async (userContent: UserContent) => {
      ...
      const runSingle = async (
        ...
        const options = { ...configuredOptions, ...configuredOptions.getProjectRuntime?.() };
  ...
  const trackedRun = useCallback(
    (content: UserContent) => runCompletionRef.current.track(() => run(content)),
    [run],
  );
  const suspendForProjectSwitch = useCallback(
    () =>
      runCompletionRef.current.suspend(() => {
        if (queueRef.current.length > 0) {
          throw new Error("Finish or clear queued messages before switching projects.");
        }
        abortRef.current?.abort();
      }),
    [],
