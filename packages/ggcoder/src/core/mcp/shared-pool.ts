...
export interface SharedAcquireOptions {
  /** Connections with different handshake identities must never be shared. */
  clientName?: string;
...
export class SharedMcpPool {
  ...
  private keyFor(config: MCPServerConfig, opts: SharedAcquireOptions): string {
    ...
    return `${config.name}\u0000${hashServerConfig(config)}\u0000${era}\u0000${prompting}\u0000${JSON.stringify(opts.clientName ?? KLEIO_PRODUCT_PROFILE.coder.mcpClientName)}`;
  }
