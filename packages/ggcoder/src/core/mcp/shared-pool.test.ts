...
import type { ToolContext } from "@kleio/agent";
...
describe("SharedMcpPool refcounting", () => {
  ...
  it("keeps distinct handshake identities in separate connections", async () => {
    const pool = newPool();
    const log: string[] = [];
    const factory = () => fakeConnector(log);
    const first = await pool.acquire(config, factory, { clientName: "first-client" });
    const second = await pool.acquire(config, factory, { clientName: "second-client" });

    expect(log).toEqual(["connect:pooled", "connect:pooled"]);
    expect(pool.size).toBe(2);
    expect(pool.refCount(config, { clientName: "first-client" })).toBe(1);
    expect(pool.refCount(config, { clientName: "second-client" })).toBe(1);
    await first.release();
    expect(pool.refCount(config, { clientName: "second-client" })).toBe(1);
    await second.release();
    expect(pool.size).toBe(0);
