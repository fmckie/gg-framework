...
export class MCPClientManager {
  ...
  constructor(opts: MCPClientManagerOptions = {}) {
    this.clientName = resolveMcpClientName(opts.clientName, opts.environment ?? process.env);
  ...
  private createClient(serverName: string, negotiate: NegotiationOptions): Client {
    ...
    if (!handler) return new Client({ name: this.clientName, version: "1.0.0" }, negotiate);
  ...
  async login(
    ...
    const server = http.createServer((req, res) => {
      ...
      res.end(
        "<html><body><h1>Login successful!</h1><p>You can close this tab and return to Kleio Coder.</p></body></html>",
      );
    ...
    try {
      ...
      const loginClient = new Client({ name: this.clientName, version: "1.0.0" });
      try {
      ...
      const verifyClient = new Client({ name: this.clientName, version: "1.0.0" });
