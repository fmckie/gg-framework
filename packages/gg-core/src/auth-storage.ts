...
import { KLEIO_PRODUCT_PROFILE } from "./product-profile.js";
...
const DUAL_AUTH_PROVIDERS: readonly DualAuthProvider[] = [
  {
    ...
    restoreHint: `Run "${LOGIN_COMMAND}" and choose Kimi OAuth to restore OAuth auth.`,
  },
  {
    provider: "xai",
    ...
    restoreHint: `Run "${LOGIN_COMMAND}" and choose Grok OAuth to restore OAuth auth.`,
  },
];
...
export class NotLoggedInError extends Error {
  ...
  constructor(provider: string) {
    super(`Not logged in to ${provider}. Run "${LOGIN_COMMAND}" to authenticate.`);
